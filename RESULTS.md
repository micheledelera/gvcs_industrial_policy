...
## §10g. The B4/B5 figures on lnX — `data/b2_report_lnx.py`, `data/plot_b2_lnx.py`

`data/b2_paths_lnX.png`. The same two panels as `b2_paths.png` but on the A0 **check**
outcome: log US imports in thousands of USD rather than the log share. Same merged design
(threshold 6+, r = 2, covariate Dec_k × Post, 1,083 treated country-sectors in 25 countries
against 2,223 strictly clean controls), same country-blocked bootstrap band.

### The levels are interpretable here, and that answers a reading question about §10e's figure

The lnS panel sits at about −7 throughout. That is not a defect: it is a log of a fraction.
exp(−7.0) = 0.00091, i.e. the mean treated country-sector holds 0.09% of US imports in its HS
sector. The number on that axis is the **mean of logs**, which tracks the median share
(0.064%) and not the mean share (1.06%) — the pooled treated share distribution has p10/p90 of
0.0016%/2.20% and 83.3% of observations below 1%, so a handful of large slices pull the
arithmetic mean up by a factor of 17 without moving the log mean. Only the vertical *distance*
between the two lines is interpretable in either figure; a gap of 0.20 log points is +22%
proportionally whether the lines sit at −7 or at +9.

On lnX the level does mean something: 8.34 in 2007 is $4.2m of US imports for the mean treated
country-sector, 9.41 in 2024 is $12.3m.

### The two outcomes agree, so A0 does no work

| | lnS (primary) | lnX (check) |
|---|---|---|
| ATT, post mean | +0.229 | **+0.234** |
| country-blocked se | 0.238 | 0.239 |
| p, t(24) | 0.346 | 0.338 |
| max pre-window imbalance | 0.02 | 0.025 |
| control-side R² | — | 0.886 |
| median per-unit pre RMSE | — | 0.291 (9% of the outcome sd) |

The two differ only by a sector-year constant and the GSC absorbs sector-year variation through
the factor structure, so near-identity was expected. The point is that the A0 choice to make
lnS primary carries no result with it.

### One substantive difference, and it cuts in favour of the estimate

§10e flagged that on lnS **59% of the gap is the counterfactual declining** rather than the
treated rising — which leaves open that the result is a denominator artefact, the synthetic's
*share* falling because its sectors grew. In dollars that concern shrinks:

| | pre → post change |
|---|---|
| treated | +0.662 log points |
| counterfactual | **+0.428** (rises, does not fall) |
| share of the gap that is counterfactual movement | **39.3%** |

The counterfactual still grows, just more slowly. So the treated group genuinely grows faster;
it is not only that its comparison group shrinks.

### Two things that do not change

1. **The timing.** ATT by year: 2018 +0.050, 2019 +0.101, then 2020 +0.171, 2021 +0.321,
   2022 +0.396, 2023 +0.280, 2024 +0.317. The effect accumulates in the COVID window, not at
   the tariff event, exactly as on lnS. The OPEN DESIGN QUESTION in `PROTOCOL_SC.md` is not
   relieved by changing the outcome — if anything it is sharpened, since the 2018–19 gap is as
   small in dollars as in shares.
2. **The size concentration of §10d**, unchanged in levels:

| quintile of 2015–17 imports | n | mean size | ATT |
|---|---|---|---|
| Q1 | 217 | $0.2m | **+0.986** |
| Q2 | 216 | $2.1m | −0.147 |
| Q3 | 217 | $8.6m | +0.024 |
...
The pooled ATT is still generated by country-sectors too small to support any aggregate claim,
and the quintile holding essentially all the trade value is +0.10.
