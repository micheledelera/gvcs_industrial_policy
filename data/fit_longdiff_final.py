...
C=(C-C.mean())/C.std()
w0=d['w'].sum(); d=d.merge(C,left_on='i',right_index=True,how='left').dropna(subset=CTRL)
...
for MEASURE in MEAS:
    ...
    print()
pd.DataFrame(rows).to_csv("longdiff_final.csv",index=False)
print("saved longdiff_final.csv")
