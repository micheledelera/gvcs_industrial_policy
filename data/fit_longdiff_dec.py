"""Long difference with the decoupling interaction.
...
for DEC in ['Dec_pre','Dec_realised']:
    ...
    for M in MEAS:
        d['IPz']=d[M]/d[M].std(); d['IPxDec']=d['IPz']*d['Dz']
        X=['IPxDec','IPz','d_pre','s1_pp']
        Mx=pd.concat([d[X],sec],axis=1)
        m=sm.WLS(d['d_post'],sm.add_constant(Mx),weights=w).fit(
            cov_type='cluster',cov_kwds={'groups':d['i']})
        print(f"{M:22s} IPxDec {m.params['IPxDec']:+.4f} (se {m.bse['IPxDec']:.4f}) "
              f"p={m.pvalues['IPxDec']:.4f}   |   IP {m.params['IPz']:+.4f} "
              f"(se {m.bse['IPz']:.4f}) p={m.pvalues['IPz']:.4f}")
        rows.append({'dec':DEC,'measure':M,'b_int':m.params['IPxDec'],'se_int':m.bse['IPxDec'],
                     'p_int':m.pvalues['IPxDec'],'b_ip':m.params['IPz'],'se_ip':m.bse['IPz'],
                     'p_ip':m.pvalues['IPz'],'N':int(m.nobs)})
pd.DataFrame(rows).to_csv("longdiff_dec_results.csv",index=False)
