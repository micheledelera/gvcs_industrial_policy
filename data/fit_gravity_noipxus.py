...
for LAG in LAGS:
    ...
    # NO IPxUS_dev / IPxUS_adv: robustness check requested by the author.
    # NB DDD_dev is a strict subset of IPxUS_dev's support (nonzero only where
    # IPxUS_dev is nonzero AND dec=1), and IPxUS_dev is significantly negative
    # (-0.038, p=0.043 full sample), so omitting it forces one coefficient to fit
    # both the general negative level and the decoupling-specific positive
    # differential -> attenuation toward zero is the PREDICTED direction.
    f = ("imports ~ DDD_dev + DDD_adv + IPxUS_chn "
    ...
    try:
        ...
        t = fit.tidy()
        print(f"\n### FULL SAMPLE, NO IPxUS -- {MEASURE}, lag {LAG}   N={fit._N:,}   cluster (i,s)")
        print(t.round(5).to_string())
        t.to_csv(f"gravity_full_noipxus_lag{LAG}.csv")
