...
# Weights must be built on the SAME outcome they are applied to. Matching the US path
# and then applying those weights to TOTAL or NON-US leaves those outcomes' own
# pre-periods unmatched, which shows up as a significant placebo. Here omega is rebuilt
# against each outcome's own 2010-2017 path.
...
MW=2.0
def run(Yout, Ypath, col_post, col_plac):
    out=[]
...
print("="*88)
print(f"SC, weights matched to each outcome OWN path   MVA_W = {MW}, zeta -> 0, identical sample")
print("="*88)
print(f"{'outcome':>10s}{'cells':>7s}{'TRUE tau':>26s}{'PLACEBO tau':>26s}")
rows=[]
for lab,Yo in OUT.items():
    o=run(Yo,Yo,'tau','plac')
...
r=pd.DataFrame(rows); r.to_csv("sc_margins_own.csv",index=False)
if len(r)==3:
